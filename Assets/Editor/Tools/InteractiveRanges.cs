...
namespace Editor.Tools
{
    public static class InteractiveRanges
    {
            ...
            float range = 0.5f, float minimumRange = 0.2f, bool takeNewRange = false)
        {
            ...
            // Rotate in local space.
            // I don't know why rotated vector is only 0.5 length so I have to multiply by two to get a 1 unit length vector.
            Vector3 rotatedVector = Quaternion.AngleAxis(semiConeDegrees, coneNormal) * localConeForward;
            // Handle is placed in world space so rotated vector must be taken back to world space.
            Vector3 handlePosition = parentTransform.TransformPoint(rotatedVector * range);
            Vector3 newHandlePosition = Handles.PositionHandle(
                handlePosition, 
                Quaternion.identity);
            Vector3 minimumRangeHandlePosition = parentTransform.TransformPoint(rotatedVector * minimumRange);
            Vector3 newMinimumRangeHandlePosition = Handles.PositionHandle(
                minimumRangeHandlePosition,
                Quaternion.identity);
            range = takeNewRange ? Vector3.Distance(position, newHandlePosition) : range;
            minimumRange = takeNewRange ? Vector3.Distance(position, newMinimumRangeHandlePosition) : minimumRange;
            // Outer arc.
            Handles.DrawWireArc(position, coneNormal, globalForward, semiConeDegrees, range);
            Handles.DrawWireArc(position, coneNormal,
                globalForward, -1 * semiConeDegrees, range);
            // Inner arc.
            Handles.DrawWireArc(position, coneNormal, globalForward, semiConeDegrees, minimumRange);
            Handles.DrawWireArc(position, coneNormal,
                globalForward, -1 * semiConeDegrees, minimumRange);
            // Lateral lines.
